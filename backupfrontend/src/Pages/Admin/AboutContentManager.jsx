...
const fetchAboutData = async () => {
  try {
    console.log("Fetching data from:", `${API_BASE_URL}/api/about`);
    const response = await axios.get(`${API_BASE_URL}/api/about`);
    return response.data;
  } catch (error) {
    console.error("Error fetching about data:", error);
...
function AboutContentManager() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (data) {
        ...
        const previews = {};
        formattedData.sections.forEach((section, index) => {
          if (section.imageUrl) previews[index] = section.imageUrl;
        });
        setImagePreview(previews);
      } else {
  ...
  const handleInputChange = (e, sectionIndex = null) => {
    const { name, value } = e.target;
    setAboutData((prev) => {
      if (sectionIndex === null) return { ...prev, [name]: value };
      const updatedSections = [...prev.sections];
  ...
  const handleImageUpload = async (e, sectionIndex) => {
    ...
    if (!validTypes.includes(file.type) || file.size > maxSize) {
      setError("Invalid file type or size. Upload JPEG, PNG, GIF, or WebP (max 5MB).");
      return;
    }
    ...
    try {
      const reader = new FileReader();
      reader.onloadend = () => {
        setImagePreview((prev) => ({ ...prev, [sectionIndex]: reader.result }));
      };
      reader.readAsDataURL(file);
  ...
  const handleSubmit = async (e) => {
    ...
    if (aboutData.sections.length === 0 || aboutData.sections.some((s) => !s.title || !s.text)) {
      setError("Each section must have a title and text.");
      return;
    }
  ...
  return (
    <div className="about-content-manager">
      ...
      <form onSubmit={handleSubmit}>
        <label>Page Title</label>
        <input type="text" name="title" value={aboutData.title} onChange={handleInputChange} required />
        ...
        {aboutData.sections.map((section, index) => (
          <div key={index}>
            <h3>Section {index + 1}</h3>
            <input type="text" name="title" placeholder="Title" value={section.title} onChange={(e) => handleInputChange(e, index)} required />
            <textarea name="text" placeholder="Text" value={section.text} onChange={(e) => handleInputChange(e, index)} required />

            <input type="file" onChange={(e) => handleImageUpload(e, index)} />
            {imagePreview[index] && <img src={imagePreview[index]} alt="Preview" />}

            <button type="button" onClick={() => removeSection(index)}>Remove Section</button>
          </div>
        ))}

        <button type="submit">{isEditing ? "Update Content" : "Create Content"}</button>
      </form>
    </div>
  );
}
